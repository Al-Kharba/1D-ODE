...
def vis(t, xall):
    ...
    # data PDF
    fig.add_trace(go.Scatter(x=data_pdf(x0), y=x0, mode='lines', line=dict(color='orange'), name='data pdf'), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(x=data_cond_pdf(x0), y=x0, mode='lines', line=dict(color='green'), name='cond data pdf'), row=1, col=1)
    ...
    for i, trajectory in enumerate(xall):
        fig.add_trace(go.Scatter(x=t, y=trajectory[:, 0], mode='lines', line=dict(color='black', width=0.2), name='ODE trajectory', showlegend=True if i == 0 else False), row=1, col=2)
